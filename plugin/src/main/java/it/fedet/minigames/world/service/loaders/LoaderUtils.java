...
import it.fedet.minigames.api.world.utils.WorldFormat;
...
import it.fedet.minigames.world.nms.CraftSlimeChunk;
...
import it.fedet.minigames.world.service.loaders.mongodb.MongoLoader;
import it.fedet.minigames.world.service.loaders.mysql.MysqlLoader;
...
public class LoaderUtils {
        ...
        if (loaderMap.containsKey(storageType)) {
            ...
            if (loader instanceof UpdatableLoader) {
                try {
                    ...
                    Logging.error(WorldService.class, "Data source " + storageType.name() + " version is " + e.getDatabaseVersion() + ", while this SWM version only supports up to version " + e.getCurrentVersion() + ".");
                    return;
                } catch (IOException ex) {
                    Logging.error(WorldService.class, "Failed to check if data source " + storageType.name() + " is updated:");
                    ex.printStackTrace();
                    return;
                }
            }
    ...
    public static CraftSlimeWorld deserializeWorld(SlimeLoader loader, String worldName, byte[] serializedWorld, WorldPropertyMap propertyMap, boolean readOnly, boolean ignoreLocked) throws IOException, CorruptedWorldException, NewerFormatException {
        DataInputStream dataStream = new DataInputStream(new ByteArrayInputStream(serializedWorld));

        try {
            byte[] fileHeader = new byte[WorldFormat.HEADER.length];
            dataStream.read(fileHeader);
            if (!Arrays.equals(WorldFormat.HEADER, fileHeader)) {
