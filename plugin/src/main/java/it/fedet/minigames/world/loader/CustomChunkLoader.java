...
import it.fedet.minigames.world.nms.chunk.NMSSlimeChunk;
...
public class CustomChunkLoader implements IChunkLoader {
    ...
    public void loadAllChunks(CustomWorldServer server) {
    ...
    private Chunk createChunk(CustomWorldServer server, SlimeChunk chunk) {
        ...
        for (int sectionId = 0; sectionId < chunk.getSections().length; sectionId++) {
            CraftSlimeChunkSection slimeSection = chunk.getSections()[sectionId];
    ...
    public void a(World world, Chunk chunk) {
        ...
        if (slimeChunk instanceof NMSSlimeChunk nmsSlimeChunk) { // In case somehow the chunk object changes (might happen for some reason)
            nmsSlimeChunk.setChunk(chunk);
        } else {
