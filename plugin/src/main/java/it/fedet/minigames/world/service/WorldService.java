...
import it.fedet.minigames.world.nms.SlimeNMS;
import it.fedet.minigames.world.nms.v1_8_R3.v1_8_R3SlimeNMS;
import it.fedet.minigames.world.service.loaders.LoaderUtils;
import it.fedet.minigames.world.service.world.WorldUnlocker;
import it.fedet.minigames.world.service.world.importer.WorldImporter;
...
public class WorldService implements IWorldService {
    ...
    public void start() {
        ...
        try {
            ...
            Logging.error(WorldService.class, ex.getMessage());
            return;
        }
        ...
        try {
            ...
            Logging.error(WorldService.class, "Failed to retrieve default world name:");
            ex.printStackTrace();
        }

        if (this.nms == null) {
            Logging.error(WorldService.class, "SlimeWorldManager failed to enable due to an unsupported Minecraft version.");
        } else {
    ...
    public SlimeWorld loadWorld(SlimeLoader loader, String worldName, boolean readOnly, WorldPropertyMap propertyMap) throws UnknownWorldException, IOException, CorruptedWorldException, NewerFormatException, WorldInUseException {
        return this.loadWorld(loader, worldName, readOnly, propertyMap, false);
    }

    public SlimeWorld loadWorld(SlimeLoader loader, String worldName, boolean readOnly, WorldPropertyMap propertyMap, boolean ignoreLocked) throws UnknownWorldException, IOException, CorruptedWorldException, NewerFormatException, WorldInUseException {
    ...
    public SlimeWorld createEmptyWorld(SlimeLoader loader, String worldName, boolean readOnly, WorldPropertyMap propertyMap) throws WorldAlreadyExistsException, IOException {
        Objects.requireNonNull(loader, "Loader cannot be null");
        Objects.requireNonNull(worldName, "World name cannot be null");
        Objects.requireNonNull(propertyMap, "Properties cannot be null");
        if (loader.worldExists(worldName)) {
            ...
            Logging.info(WorldService.class, "Creating empty world " + worldName + ".");
            long start = System.currentTimeMillis();
            ...
            Logging.info(WorldService.class, "World " + worldName + " created in " + (System.currentTimeMillis() - start) + "ms.");
            return world;
        }
    }

    private WorldPropertyMap propertiesToMap(SlimeWorld.SlimeProperties properties) {
        WorldPropertyMap propertyMap = new WorldPropertyMap();
        propertyMap.setInt(WorldProperties.SPAWN_X, (int) properties.getSpawnX());
        propertyMap.setInt(WorldProperties.SPAWN_Y, (int) properties.getSpawnY());
        propertyMap.setInt(WorldProperties.SPAWN_Z, (int) properties.getSpawnZ());
        propertyMap.setString(WorldProperties.DIFFICULTY, Difficulty.getByValue(properties.getDifficulty()).name());
        propertyMap.setBoolean(WorldProperties.ALLOW_MONSTERS, properties.allowMonsters());
        propertyMap.setBoolean(WorldProperties.ALLOW_ANIMALS, properties.allowAnimals());
        propertyMap.setBoolean(WorldProperties.PVP, properties.isPvp());
        propertyMap.setString(WorldProperties.ENVIRONMENT, properties.getEnvironment());
        return propertyMap;
    }
