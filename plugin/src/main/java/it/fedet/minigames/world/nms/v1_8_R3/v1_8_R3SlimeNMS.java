...
public class v1_8_R3SlimeNMS implements SlimeNMS {
    ...
    public void setDefaultWorlds(SlimeWorld normalWorld, SlimeWorld netherWorld, SlimeWorld endWorld) {
        if (normalWorld != null) {
            World.Environment env = Environment.valueOf(normalWorld.getPropertyMap().getString(WorldProperties.ENVIRONMENT).toUpperCase());
        ...
        if (netherWorld != null) {
            World.Environment env = Environment.valueOf(netherWorld.getPropertyMap().getString(WorldProperties.ENVIRONMENT).toUpperCase());
            this.defaultNetherWorld = new CustomWorldServer((CraftSlimeWorld) netherWorld, new CustomDataManager(netherWorld), env.getId());
        }

        if (endWorld != null) {
            World.Environment env = Environment.valueOf(endWorld.getPropertyMap().getString(WorldProperties.ENVIRONMENT).toUpperCase());
    ...
    public CompletableFuture<World> addWorldToServerList(Object worldObject) {
        ...
        if (!(worldObject instanceof WorldServer)) {
            ...
            if (w != null) {
                ...
                Logging.info(WorldService.class, "Loading world " + worldName);
                ...
                Logging.info(WorldService.class,"World " + worldName + " loaded in " + (System.currentTimeMillis() - startTime) + "ms.");
